...
export const ItemAction: React.FC<Props> = ({ type, id }) => {
  ...
  const renderCorrectText = (isAdded: boolean, type: 'favourites' | 'watchlist') => {
    ...
    return isAdded ? `Saved in ${type}` : `Add to ${type}`;
  };
  ...
  return (
    <div className="flex flex-col absolute opacity-100 top-0 left-2 group-hover:opacity-100">
      <button
        type="button"
        className="flex gap-2 md:gap-1 items-center opacity-0 group-hover:opacity-100 duration-700 "
      ...
      >
        ...
        <span 
          className="opacity-0 translate-x-[-10px] text-gray-300 text-base peer-hover:opacity-100 peer-hover:translate-x-0 duration-500 truncate">
      ...
      >
        ...
        <span className="opacity-0 translate-x-[-10px] text-gray-300 text-base peer-hover:opacity-100 peer-hover:translate-x-0 duration-500 truncate">
